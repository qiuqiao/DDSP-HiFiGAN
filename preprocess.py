...
import argparse
import shutil
from pc_ddsp.logger import utils
from tqdm import tqdm
from librosa.filters import mel as librosa_mel_fn
from pc_ddsp.logger.utils import traverse_dir
        ...
        if len(f0[~uv]) > 0:
            ...
            with torch.no_grad():
                signal, _, (s_h, s_n) = model(
                    torch.from_numpy(mel).to(device).unsqueeze(0).type(torch.float32),
                    torch.from_numpy(f0).to(device).unsqueeze(0).unsqueeze(-1).type(torch.float32))
            mel = stft.get_mel(torch.tensor(signal)).squeeze(0).cpu().numpy()
...
if __name__ == '__main__':
    ...
    # load config
    args = utils.load_config(os.path.join(os.path.split(cmd.model)[0],'config.yaml'))
