...
class Enhancer:
    def __init__(self, enhancer_ckpt, device=None):
        if device is None:
    ...
    def enhance(self,
        ...
        # enhancer start time 
        start_frame = int(silence_front * sample_rate / hop_size)#从第几帧开始不是静音
        ...
        adaptive_factor = 2 ** ( -adaptive_key / 12)#转换为单位为频率因数
        adaptive_sample_rate = 100 * int(np.round(self.enhancer_sample_rate / adaptive_factor / 100))
        #其实只是self.enhancer_sample_rate / adaptive_factor罢了，其他的代码是确保自适应采样率为 100 的倍数
